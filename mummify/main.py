...
def switch(id):
    ...
    run('git --work-tree=. --git-dir=.mummify merge -s ours --no-commit master 2>/dev/null')
    run(f'git --work-tree=. --git-dir=.mummify checkout logger {LOGFILE} 2>/dev/null')
    run(f'git --work-tree=. --git-dir=.mummify commit -m "switch-{id}"', silent=True)
    run('git --work-tree=. --git-dir=.mummify checkout master', silent=True)
    run('git --work-tree=. --git-dir=.mummify merge switcher 2>/dev/null')
